...
func PostpathHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/%s", cfg.BaseURL, path, queryString)
...
func CreatePostpathTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_path",
		mcp.WithDescription(""),
		mcp.WithString("path", mcp.Required(), mcp.Description("")),
		mcp.WithString("jcr:primaryType", mcp.Required(), mcp.Description("")),
		mcp.WithString(":name", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    PostpathHandler(cfg),
	}
}
