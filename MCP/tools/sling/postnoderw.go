...
func PostnoderwHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["addMembers"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("addMembers=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/%s/%s.rw.html%s", cfg.BaseURL, path, name, queryString)
...
func CreatePostnoderwTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_path_name.rw.html",
		mcp.WithDescription(""),
		mcp.WithString("path", mcp.Required(), mcp.Description("")),
		mcp.WithString("name", mcp.Required(), mcp.Description("")),
		mcp.WithString("addMembers", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    PostnoderwHandler(cfg),
	}
}
