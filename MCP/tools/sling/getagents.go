...
func GetagentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/etc/replication/agents.%s.-1.json", cfg.BaseURL, runmode)
...
func CreateGetagentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_etc_replication_agents.runmode.-1.json",
		mcp.WithDescription(""),
