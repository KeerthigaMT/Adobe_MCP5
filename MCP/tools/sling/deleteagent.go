...
func DeleteagentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/etc/replication/agents.%s/%s", cfg.BaseURL, runmode, name)
...
func CreateDeleteagentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_etc_replication_agents.runmode_name",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeleteagentHandler(cfg),
	}
}
