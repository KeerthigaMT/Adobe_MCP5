...
func GetpackagefilterHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/etc/packages/%s/%s-%s.zip/jcr:content/vlt:definition/filter.tidy.2.json", cfg.BaseURL, group, name, version)
...
func CreateGetpackagefilterTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_etc_packages_group_name-version.zip_jcr:content_vlt:definition_filter.tidy.2.json",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetpackagefilterHandler(cfg),
	}
}
